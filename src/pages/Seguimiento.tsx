...
const Badge = ({ estado, clickable = false, onClick }: { estado: string; clickable?: boolean; onClick?: () => void }) => {
  ...
  return (
    <span title={estado} className={`${base} ${cursor}`} onClick={clickable && estado !== "no-aplica" ? onClick : undefined}>
...
function TarjetaProducto({ pedido }: { pedido: PedidoSeguimiento }) {
  const cantidad = pedido.modo_cantidad === "kilo" && pedido.kilogramos_orden
    ...
    : pedido.cantidad_orden ? pedido.cantidad_orden.toLocaleString("es-MX") : "—";
  return (
...
function ModalProcesoIndividual({
  ...
  pedido: PedidoSeguimiento; nombreProceso: string; onClose: () => void; onActualizar: () => void;
}) {
  ...
  const handleAbrirFinalizar = () => {
    ...
    if (proc?.registro) {
      campos.forEach(c => { if (proc.registro[c.key] != null) preFill[c.key] = proc.registro[c.key]; });
    }
  ...
  return (
    <div className="space-y-4 min-w-[440px]">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
          <p className="text-xs font-semibold text-blue-800 uppercase tracking-wide mb-2">
            ...
            <span className="normal-case font-normal text-blue-600">
              {calculados.orientacion === "horizontal" ? "Extrusión horizontal (fuelle de fondo)" : "Extrusión vertical (fuelle lateral)"}
            </span>
          </p>
        ...
        <>
          ...
          {!esActual && proc.estado !== "terminado" && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 text-center">
              <p className="text-yellow-800 text-sm">Este proceso aún no es el actual. Deben completarse los procesos anteriores primero.</p>
            </div>
          )}
          ...
          {puedeFinalizar && accion !== "iniciar" && (
            <>
              {accion !== "finalizar" ? (
                <button onClick={handleAbrirFinalizar} className="w-full py-2.5 bg-green-600 hover:bg-green-700 text-white text-sm font-semibold rounded-lg transition-colors">
                  ...
                  {campos.map(campo => (
                    <div key={campo.key}>
                      ...
                      <input
                        ...
                        onChange={e => !campo.readOnly && setFormDatos(prev => ({ ...prev, [campo.key]: e.target.value }))}
                        readOnly={campo.readOnly}
                        className={`w-full px-3 py-1.5 border rounded text-sm focus:outline-none ${campo.readOnly ? "bg-blue-50 border-blue-200 text-blue-700 font-semibold cursor-not-allowed" : "border-gray-300 focus:ring-2 focus:ring-green-400"}`}
                        placeholder="0"
                      />
                    </div>
                  ))}
                  <div className="flex gap-2 pt-1">
                    <button onClick={() => { setAccion(null); setFormDatos({}); }} className="flex-1 py-2 border border-gray-300 text-gray-600 text-sm rounded-lg hover:bg-gray-50">Cancelar</button>
                    <button onClick={handleFinalizar} disabled={guardando} className="flex-1 py-2 bg-green-600 hover:bg-green-700 disabled:bg-green-400 text-white text-sm font-semibold rounded-lg flex items-center justify-center gap-2">
      ...
      <div className="flex justify-end pt-2 border-t border-gray-100">
        <button onClick={onClose} className="px-5 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 text-sm">Cerrar</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex items-center gap-1.5 justify-center">
      ...
      <button onClick={handleDescargar} disabled={descargando}
        ...
        {descargando ? <div className="w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <IconoPdf />}
        PDF
      </button>
    </div>
  );
...
function RenderOrdenProduccion({ pedido }: { pedido: PedidoSeguimiento }) {
  if (!pedido.puede_pdf || !pedido.no_produccion) {
    const tooltip = !pedido.anticipo_cubierto && !pedido.diseno_aprobado ? "Falta anticipo y diseño"
      : !pedido.anticipo_cubierto ? "Falta anticipo" : "Diseño no aprobado";
    return (
      <span title={tooltip} className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-gray-200 text-gray-400 text-xs rounded cursor-not-allowed">
        <IconoPdf /> PDF
      </span>
    );
  }
...
const COLUMNAS = ["Fecha", "N° Pedido", "Cliente", "Tipo", "Anticipo", "Diseño", "Orden", "Ext", "Imp", "Bol", "Asa", "Pago", "Envío"];
const COLS_CENTRADAS = new Set(["Anticipo", "Diseño", "Orden", "Ext", "Imp", "Bol", "Asa", "Pago", "Envío"]);

const renderThead = (oscuro = false) => (
  <thead className={oscuro ? "bg-gray-900 text-white" : "bg-gray-100 border-b border-gray-200"}>
    <tr>
      {COLUMNAS.map(h => (
        <th key={h} className={`px-3 py-2 text-xs font-semibold uppercase tracking-wider ${oscuro ? "text-white" : "text-gray-700"} ${COLS_CENTRADAS.has(h) ? "text-center" : "text-left"}`}>
          {h}
        </th>
      ))}
    </tr>
...
export default function Seguimiento() {
  ...
  const cargar = async () => {
    try { setCargando(true); setError(null); setPedidos(await getSeguimiento()); }
    catch { setError("No se pudo cargar el seguimiento."); }
    finally { setCargando(false); }
  };

  const pedidosFiltrados = filtroTipo === "todos" ? pedidos : pedidos.filter(p => (p.tipo_producto || "").toLowerCase().includes(filtroTipo));
  ...
  const renderFila = (pedido: PedidoSeguimiento, grande = false) => {
    ...
    const abrirProceso = (nombreProceso: string) => { if (!tieneOrden) return; setModalProceso({ pedido, nombreProceso }); };
    return (
      <tr key={`${pedido.no_pedido}-${pedido.no_produccion}`} className="hover:bg-gray-50 transition-colors border-t border-gray-200">
        <td className={`${px} ${txt} text-gray-900 whitespace-nowrap`}>{new Date(pedido.fecha).toLocaleDateString("es-MX")}</td>
        <td className={`${px} ${txt} font-medium text-blue-600 whitespace-nowrap`}>PED-{pedido.no_pedido}</td>
        <td className={`${px} ${txt} text-gray-900`}>{pedido.cliente}</td>
        <td className={`${px}`}>
          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800 capitalize">{pedido.tipo_producto || "—"}</span>
        </td>
        ...
        <td className={`${px} text-center`}><Badge estado={extEstado} clickable={tieneOrden && extEstado !== "no-aplica"} onClick={() => abrirProceso("extrusion")} /></td>
        <td className={`${px} text-center`}><Badge estado={impEstado} clickable={tieneOrden && impEstado !== "no-aplica"} onClick={() => abrirProceso("impresion")} /></td>
        <td className={`${px} text-center`}><Badge estado={bolEstado} clickable={tieneOrden && bolEstado !== "no-aplica"} onClick={() => abrirProceso("bolseo")} /></td>
        <td className={`${px} text-center`}><Badge estado={asaEstado} clickable={tieneOrden && asaEstado !== "no-aplica"} onClick={() => abrirProceso("asa_flexible")} /></td>
  ...
  if (pantallaCompleta) return (
    <div className="p-6 min-h-screen bg-gray-50">
      <button onClick={() => setPantallaCompleta(false)} className="mb-4 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 font-medium flex items-center gap-2">
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">{renderThead(true)}<tbody>{pedidosFiltrados.map(p => renderFila(p, true))}</tbody></table>
        </div>
      </div>
    </div>
  );
      ...
      <div className="bg-white rounded-lg shadow p-4 mb-6">
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <div className="flex gap-2 flex-wrap">
            {[{ key: "todos", label: "Todos" }, { key: "plástico", label: "Plástico" }, { key: "papel", label: "Papel" }, { key: "cartón", label: "Cartón" }].map(f => (
              <button key={f.key} onClick={() => setFiltroTipo(f.key)}
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${filtroTipo === f.key ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}>
                {f.label}
              </button>
            ))}
          </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <div className="p-4 border-b border-gray-200 flex justify-between items-center">
          <h2 className="text-lg font-semibold text-gray-900">
            Lista de Órdenes
            <span className="ml-2 text-sm font-normal text-gray-500">({pedidosFiltrados.length} orden{pedidosFiltrados.length !== 1 ? "es" : ""})</span>
          </h2>
          <button onClick={() => setPantallaCompleta(true)} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium flex items-center gap-2">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">{renderThead()}<tbody>{pedidosFiltrados.map(p => renderFila(p))}</tbody></table>
        </div>
